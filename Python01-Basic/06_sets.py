...
intersection, difference, symmetric difference, subset, super set and disjoint set among sets.
"""
...
# remove() method will raise errors, so it is good to check if the item exist in the given set.
...
# It means that it returns a set that contains all items from both sets, except items 
# that are present in both sets, mathematically: (A\B) ∪ (B\A)
...
# are 7 steps ahead in to your way to greatness. 
